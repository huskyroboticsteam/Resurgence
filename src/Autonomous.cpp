...
//target gps position
const PointXY gpsTarget = getGpsTarget();
...
std::pair<float, float> Autonomous::stateForwards (float currHeading, std::pair<float, float> directions) {
    if(!lidarSees()) { //keep moving forwards
...
std::pair<float, float> Autonomous::stateTurn (float currHeading, std::pair<float, float> directions) {
    if(currHeading == targetHeading) { //done turning
        return stateForwards(currHeading, directions);
    }
    else { //find heading to turn to
        if(state != 0) { //only doing calculations if just starting to turn
            if(turnDirection == 1) {//turn right
                ...
                PointXY robotPos = getGPS();
                float x = gpsTarget.x - robotPos.x;
                float y = gpsTarget.y - robotPos.y;
...
std::pair<float, float> Autonomous::stateBackwards (float currHeading, std::pair<float, float> directions) {
    if(lidarSees()) { //back up
        directions = std::make_pair(currHeading, -1);
        state = -1;
        return directions;
    }
