...
namespace robot {
can_motor::can_motor(robot::types::motorid_t motor, can::devicegroup_t group, bool hasPosSensor,
					 can::deviceserial_t serial, double pos_pwm_scale, double neg_pwm_scale)
	: base_motor(motor, hasPosSensor),
	  device_id(std::make_pair(group, serial)),
...
void can_motor::setMotorPower(double power) {
	...
	power *= scale;
	can::motor::setMotorPower(device_id, power);
}
...
void can_motor::setMotorPos(int32_t targetPos) {
	...
	can::motor::setMotorPIDTarget(device_id, targetPos);
}

types::DataPoint<int32_t> can_motor::getMotorPos() const {
	return can::motor::getMotorPosition(device_id);
}
...
void can_motor::ensureMotorMode(robot::types::motorid_t motor, can::motor::motormode_t mode) {
	if (!motor_mode || motor_mode.value() != mode) {
		...
		can::motor::setMotorMode(device_id, mode);
	}
}
} // namespace robot
