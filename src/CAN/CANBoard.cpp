...
CANBoard::CANBoard(robot::types::boardid_t board_id, CANDevice_t device)
	...
	if (device.motorDomain) {
		if (auto it = robot::boardInversionMap.find(board_id);
			it != robot::boardInversionMap.end()) {
		...
		nlohmann::json endpoint =
			getEndpoint(this->board_id, "axis0.controller.config.vel_limit");
		if (endpoint != nullptr) {
			...
			addDirectReadCallback(this->device, endpoint_id, callback);
...
void CANBoard::setMotorPower(double power) {
	if (!this->device.motorDomain) {
		LOG_F(WARNING, "setMotorPower called for %s board not in motor domain!",
			  util::to_string(this->board_id).c_str());
		return;
	}

	if (power == 0.0) {
		if (this->board_id == robot::types::boardid_t::shoulder ||
			this->board_id == robot::types::boardid_t::elbow) {
			// Set brake
		...
		// Make CANPacket_t
		CANPacket_t p = CANMotorPacket_BLDC_SetInputVelocity(Constants::JETSON_DEVICE,
															 this->device, 0.0f, 0.0f);
		...
		// Make CANPacket_t
		CANPacket_t p = CANMotorPacket_BLDC_SetInputVelocity(Constants::JETSON_DEVICE,
															 this->device, input_vel, 0.0f);

		// Send packet
		sendCANPacket(p);

		// Double-check velocity set correctly
		nlohmann::json endpoint = getEndpoint(this->board_id, "axis0.controller.input_vel");
		if (endpoint != nullptr) {
			endpointid_t endpoint_id = endpoint["id"];
			auto callback = [=](auto decoded, std::unique_lock<std::shared_mutex> lock) {
...
void CANBoard::setMotorState(can::motor::axis_state_t state) {
	if (!this->device.motorDomain) {
		LOG_F(WARNING, "setMotorState called for %s board not in motor domain!",
			  util::to_string(this->board_id).c_str());
		return;
	}
	...
	CANPacket_t p =
		CANMotorPacket_BLDC_SetAxisState(Constants::JETSON_DEVICE, this->device, axis_state);
	sendCANPacket(p);
}

void CANBoard::setMotorVel(int8_t velocity) {
	if (!this->device.motorDomain) {
		LOG_F(WARNING, "setMotorVel called for %s board not in motor domain!",
			  util::to_string(this->board_id).c_str());
		return;
	}
...
void CANBoard::setStepperRevs(float revs) {
	if (!this->device.motorDomain) {
		LOG_F(WARNING, "setStepperRevs called for %s board not in motor domain!",
			  util::to_string(this->board_id).c_str());
		return;
	}

	CANPacket_t p =
		CANMotorPacket_Stepper_DriveRevolutions(Constants::JETSON_DEVICE, this->device, revs);
	sendCANPacket(p);
}

void CANBoard::setActuator(int8_t out) {
	if (!this->device.peripheralDomain) {
		LOG_F(WARNING, "setActuator called for %s board not in peripheral domain!",
			  util::to_string(this->board_id).c_str());
		return;
	}
	...
	CANPacket_t p =
		CANPeripheralPacket_SetLinearActuator(Constants::JETSON_DEVICE, this->device, 2, out);
	sendCANPacket(p);
}
...
void CANBoard::setBrake(uint8_t state) {
	...
	if (it == robot::boardBrakeIDMap.end()) {
		LOG_F(WARNING, "setBrake called for %s that does not have a brake!",
			  util::to_string(this->board_id).c_str());
		return;
	}
...
void CANBoard::setPWMDutyCycle(uint8_t peripheralID, float dutyCycle) {
	CANPacket_t p = CANPeripheralPacket_SetPWMDutyCycle(Constants::JETSON_DEVICE,
														CANDevice_t{1, 1, 0, CAN_UUID_HAND},
														peripheralID, dutyCycle);
	sendCANPacket(p);
}
...
void CANBoard::read(endpointid_t endpoint) {
	CANPacket_t p =
		CANMotorPacket_BLDC_DirectRead(Constants::JETSON_DEVICE, this->device, endpoint);
	sendCANPacket(p);
}
