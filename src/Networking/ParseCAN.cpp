...
// Expected JSON format of Globals::status_data: each key is a string from one of the group
// lists below which identifies the device. The value is another JSON object containing one or
// more keys corresponding to telemetry measurements.
...
extern const std::vector<std::string> power_group = {
	"power_broadcast", "battery", "chassis_main", "chassis_drive_l",
	"chassis_drive_r", "arm_1",	  "arm_2",		  "science"};
extern const std::vector<std::string> motor_group = {
	"motor_broadcast",	"arm_base",			 "shoulder",		"elbow",
	"forearm",			"diffleft",			 "diffright",		"hand",
	"front_left_wheel", "front_right_wheel", "back_left_wheel", "back_right_wheel",
	"science"};
extern const std::vector<std::string> telemetry_group = {"telemetry_broadcast", "localization",
														 "imu", "temperature", "science"};
extern const std::vector<std::string> gpio_group = {"gpio_broadcast"};

extern const std::vector<std::vector<std::string>> can_groups = {
	broadcast_group, reserved_group,  master_group, power_group,
	motor_group,	 telemetry_group, gpio_group};

extern const std::vector<std::string> telem_types = {"voltage",
													 "current",
													 "power_rail_state",
													 "temperature",
													 "angular_position",
...
const std::string getDeviceTelemetryName(CANPacket& p) {
	...
	if (device_group < can_groups.size() && device_serial < can_groups[device_group].size()) {
