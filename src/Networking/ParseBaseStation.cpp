...
bool ParseBaseStationPacket(char const* buffer) {
	...
	if (type == "ik") {
		...
		success = (robot::setCmdVel(0, 0) == 1.0);
...
bool ParseEmergencyStop(json& message) {
	CANPacket p;
	AssembleGroupBroadcastingEmergencyStopPacket(&p, DEVICE_GROUP_MOTOR_CONTROL,
												 ESTOP_ERR_GENERAL);
	sendCANPacket(p);
	...
	for (uint8_t serial = 1; serial < 12; serial++) {
		AssembleEmergencyStopPacket(&p, DEVICE_GROUP_MOTOR_CONTROL, serial, ESTOP_ERR_GENERAL);
		sendCANPacket(p);
	}
	bool success = (robot::setCmdVel(0, 0) == 1.0);
	Globals::E_STOP = true;
	bool release;
	try {
...
bool ParseDrivePacket(json& message) {
	...
	double scale_factor = robot::setCmdVel(lr * DEFAULT_TH_VEL, fb * DEFAULT_X_VEL);
	if (scale_factor < 1.0) {
