...
bool ParseIKPacket(json& message) {
	...
	if (message["wrist_base_target"] != nullptr) {
		...
		double cosElbowAngle = (crossSection * crossSection - ELBOW_LENGTH * ELBOW_LENGTH -
		...
		if (shoulderAngle > Constants::SHOULDER_MAX ||
			shoulderAngle < Constants::SHOULDER_MIN) {
		...
		json base_packet = {
			{"type", "motor"}, {"motor", "arm_base"}, {"PID target", radToInt(baseAngle, 1)}};
		...
		json elbow_packet = {
			{"type", "motor"}, {"motor", "elbow"}, {"PID target", radToInt(elbowAngle, 3)}};
