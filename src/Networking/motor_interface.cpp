...
bool ParseMotorPacket(json& message) {
	...
	for (const std::string key : operation_modes) {
		...
		if (key == "PWM target") {
			...
			if (!curr_md.is_null())
				current_millideg = curr_md;
			double speed = message[key];
			int millideg_per_control_loop =
				(incremental_pid_scales.at(motor) / (2 * M_PI) * 360 * 1000) * speed /
				CONTROL_HZ;
			if (current_millideg != millideg_per_control_loop) {
				log(LOG_DEBUG, "Current mdeg is %d vs asked %d\n", current_millideg,
					millideg_per_control_loop);
				Globals::status_data[motor]["millideg_per_control_loop"] =
					millideg_per_control_loop;
