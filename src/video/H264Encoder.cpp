...
std::vector<std::basic_string<uint8_t>> H264Encoder::encode_frame(const cv::Mat& frame) {
    if (!encoder) {
        ...
        encoder = std::make_unique<h264encoder::Encoder>(size.width, size.height, size.width, size.height, fps);
    }
    std::vector<std::basic_string<uint8_t>> nals;
    auto uselessBoolean = false;
    auto frame_size = encoder->encode(frame.data, &uselessBoolean);
    for (auto i = 0; i  < encoder->num_nals; i++) {
        std::basic_string<uint8_t> data(encoder->nals[i].p_payload, encoder->nals[i].i_payload);
        nals.push_back(data);        
    }
    return nals;
}
