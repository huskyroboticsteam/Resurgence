...
std::string buildPipelinePrefix(
	int width,
	int height,
	int fps,
...
void logBusMessages(
	GstElement* pipeline,
	const char* context,
	bool includeStateChanges = false
) {
	if (!pipeline) {
		return;
	}
	...
	while (true) {
		...
		case GST_MESSAGE_ERROR: {
			...
			LOG_F(ERROR,
				  "%s: GStreamer error from %s: %s%s%s",
				  context,
		...
		case GST_MESSAGE_WARNING: {
			...
			LOG_F(WARNING,
				  "%s: GStreamer warning from %s: %s%s%s",
				  context,
				  GST_OBJECT_NAME(msg->src),
				  err ? err->message : "unknown",
				  debug ? " | debug: " : "",
...
std::vector<EncoderCandidate> buildPipelineCandidates(
	int width,
	int height,
	int fps,
	int bitrate,
	...
	std::stringstream jetsonH265Config;
	jetsonH265Config
		<< "videoconvert ! "
		...
		<< "nvv4l2h265enc "
		<< "bitrate=" << bitrate << " "
		<< "iframeinterval=" << std::max(1, fps) << " "
		<< "insert-sps-pps=true ! ";

	std::stringstream softwareH265Config;
	softwareH265Config
		<< "videoconvert ! "
		...
		<< "key-int-max=" << std::max(1, fps) << " "
		<< "bitrate=" << std::max(1, bitrate / 1000) << " ! ";

	return {
		{
			"nvv4l2h265enc",
			prefix + jetsonH265Config.str() + suffix,
			true
		},
		{
			"x265enc",
			prefix + softwareH265Config.str() + suffix,
			false
		},
	};
}
...
H265NVENCEncoder::H265NVENCEncoder(int fps, int bitrate)
	: _fps(std::max(1, fps)),
	  _bitrate(std::max(1, bitrate)) {}
...
void H265NVENCEncoder::initializePipeline(int width, int height) {
	...
	const auto candidates = buildPipelineCandidates(
		width,
		height,
		_fps,
		_bitrate,
	...
	for (const auto& candidate : candidates) {
		...
		LOG_F(INFO,
			  "Trying %s camera H.265 encoder candidate: %s",
			  mode,
			  candidate.label.c_str());

		LOG_F(INFO,
			  "H.265 encoder pipeline candidate (%s): %s",
			  candidate.label.c_str(),
		...
		if (!pipeline) {
			std::string errorMsg =
				parseError ? parseError->message : "unknown parse failure";

			LOG_F(WARNING,
				  "Failed to create %s camera H.265 encoder pipeline (%s): %s",
				  mode,
				  candidate.label.c_str(),
				  errorMsg.c_str());
		...
		GstElement* appsrc = gst_bin_get_by_name(
			GST_BIN(pipeline),
			srcName.c_str()
		);
		...
		if (!appsrc || !appsink) {
			LOG_F(WARNING,
				  "Failed to locate appsrc/appsink for %s camera H.265 encoder (%s)",
				  mode,
				  candidate.label.c_str());
		...
		if (stateChange == GST_STATE_CHANGE_FAILURE) {
			logBusMessages(pipeline, "H.265 encoder startup failure", true);

			LOG_F(WARNING,
				  "Failed to start %s camera H.265 encoder (%s)",
				  mode,
				  candidate.label.c_str());
		...
		logBusMessages(pipeline, "H.265 encoder startup", true);
		...
		LOG_F(INFO,
			  "H.265 encoder startup state result (%s): result=%d current=%s pending=%s",
			  candidate.label.c_str(),
		...
		LOG_F(INFO,
			  "Initialized %s camera H.265 encoder: %s (%dx%d@%d, bitrate=%d)",
			  mode,
		...
		if (!candidate.hardware) {
			LOG_F(WARNING,
				  "Camera stream is using SOFTWARE H.265 encoding (%s)",
				  _active_encoder_label.c_str());
		}
	...
	throw std::runtime_error(
		"Failed to initialize camera H.265 encoder pipeline. " + errors
	);
}

std::vector<std::basic_string<uint8_t>>
H265NVENCEncoder::encode_frame(const cv::Mat& frame) {
	...
	if (frame.type() != CV_8UC3) {
		LOG_F(WARNING,
			  "Camera H.265 encoder expects CV_8UC3 BGR frame, got type=%d",
			  frame.type());
		return nalUnits;
	}
	...
	if (!_pipeline) {
		try {
			...
			LOG_F(ERROR,
				  "Failed to initialize camera H.265 encoder pipeline: %s",
				  e.what());
			_init_failed = true;
			return nalUnits;
		}
	...
	if (frame.cols != _width || frame.rows != _height) {
		LOG_F(WARNING,
			  "Camera H.265 encoder resolution changed from %dx%d to %dx%d; "
			  "dropping frame",
			  _width,
			  _height,
			  frame.cols,
	...
	const size_t frameBytes =
		static_cast<size_t>(
			contiguous.cols * contiguous.rows * contiguous.elemSize()
		);

	GstBuffer* inputBuffer =
		gst_buffer_new_allocate(nullptr, frameBytes, nullptr);

	if (!inputBuffer) {
		LOG_F(ERROR, "Failed to allocate GstBuffer for camera H.265 frame");
		return nalUnits;
	}
	...
	if (!gst_buffer_map(inputBuffer, &writeMap, GST_MAP_WRITE)) {
		LOG_F(ERROR, "Failed to map GstBuffer for camera H.265 frame");
	...
	GstClockTime frameDuration =
		gst_util_uint64_scale_int(1, GST_SECOND, _fps);
	...
	GstFlowReturn pushStatus =
		gst_app_src_push_buffer(GST_APP_SRC(_appsrc), inputBuffer);

	if (pushStatus != GST_FLOW_OK) {
		logBusMessages(_pipeline, "H.265 encoder push failure");

		LOG_F(ERROR,
			  "Camera H.265 encoder appsrc push failed with status=%d",
			  pushStatus);
	...
	if (!sample) {
		...
		logBusMessages(_pipeline, "H.265 encoder empty pull");

		if (_consecutive_empty_pulls <= 10 ||
			(_consecutive_empty_pulls % 120) == 0) {
			GstState currentState = GST_STATE_NULL;
			GstState pendingState = GST_STATE_NULL;
			...
			LOG_F(WARNING,
				  "Camera H.265 encoder (%s) produced no output for %u "
				  "consecutive frames (pipeline=%s pending=%s)",
				  _active_encoder_label.c_str(),
				  _consecutive_empty_pulls,
				  gst_element_state_get_name(currentState),
				  gst_element_state_get_name(pendingState));
	...
	if (outputBuffer && gst_buffer_map(outputBuffer, &readMap, GST_MAP_READ)) {
		if (_frame_counter <= 5) {
			LOG_F(INFO,
				  "Camera H.265 encoder (%s) produced sample of %zu bytes "
				  "on frame %" PRIu64,
				  _active_encoder_label.c_str(),
