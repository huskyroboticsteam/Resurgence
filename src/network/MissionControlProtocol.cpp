...
MissionControlProtocol::MissionControlProtocol(SingleClientWSServer& server, rtc::PeerConnection& UDPConnection)
	: WebSocketProtocol(Constants::MC_PROTOCOL_NAME), _server(server),
	  _camera_stream_task(UDPConnection), _telem_report_task(server), _arm_ik_task(server),
	  _autonomous_task() {
